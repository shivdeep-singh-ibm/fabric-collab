...
func TestBlockPullerFailures(t *testing.T) {
	...
	failStream := func(osn *deliverServer, _ *replication.BlockPuller) {
	...
	malformBlockSignatureAndRecreateOSNBuffer := func(osn *deliverServer, bp *replication.BlockPuller) {
	...
	noFailFunc := func(_ *deliverServer, _ *replication.BlockPuller) {}

	recover := func(osn *deliverServer, bp *replication.BlockPuller) func(entry zapcore.Entry) error {
	...
	for _, testCase := range []struct {
		name       string
		logTrigger string
		beforeFunc func(*deliverServer, *replication.BlockPuller)
		failFunc   func(*deliverServer, *replication.BlockPuller)
	}{
		{
			name:       "failure at probe",
			logTrigger: "skip this for this test case",
			beforeFunc: func(osn *deliverServer, bp *replication.BlockPuller) {
				failStream(osn, nil)
...
func TestImpatientStreamFailure(t *testing.T) {
	...
	newStream := replication.NewImpatientStream(conn, time.Millisecond*100)
	defer conn.Close()
	// Shutdown the OSN
	osn.stop()
