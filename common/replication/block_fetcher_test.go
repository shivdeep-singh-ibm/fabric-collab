...
func TestBlockFetcherHappyPath(t *testing.T) {
	...
	bf.BlockSourceFactory = func(c replication.FetcherConfig) replication.BlockSource {
	...
	bf.Config.Endpoints = []replication.EndpointCriteria{{Endpoint: "localhost:5100"}, {Endpoint: "localhost:5200"}}
...
func TestBlockFetcherNodeOffline(t *testing.T) {
	...
	bf.ShuffleTimeout = time.Duration(0)
	bf.Logger = flogging.MustGetLogger("test")
	bf.Config.Endpoints = []replication.EndpointCriteria{
...
func TestBlockFetcherBFTBehaviorBlockWithhold(t *testing.T) {
	...
	lock := sync.Mutex{}
	bf.AttestationSourceFactory = func(c replication.FetcherConfig) replication.AttestationSource {
		lock.Lock()
		defer lock.Unlock()
...
func newAttestationPuller(dialer *countingDialer, orderers ...string) *replication.AttestationPuller {
	return &replication.AttestationPuller{
		...
		Config: replication.FetcherConfig{Channel: "mychannel", Signer: &mocks.SignerSerializer{}, TLSCert: []byte{}, Dialer: dialer, Endpoints: endpointCriteriaFromEndpoints(orderers...), FetchTimeout: time.Second * 10},
	}
}
...
func TestAttestationPullerPullAttestationsDialFailure(t *testing.T) {
	...
	ap := &replication.AttestationPuller{
		Logger: flogging.MustGetLogger("test"),
