...
// BFTEnabledInConfig takes a config block as input and returns true if consenter type is BFT
func BFTEnabledInConfig(block *common.Block, bccsp bccsp.BCCSP) bool {
	bftEnabled, _, _, _ := getConsentersAndPolicyFromConfigBlock(block, bccsp)
	return bftEnabled
}
